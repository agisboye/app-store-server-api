...
This package is [ESM-only](https://nodejs.org/api/esm.html). Load it with `import` (or a dynamic `import()` from CommonJS).
...
const api = new AppStoreServerAPI(KEY, KEY_ID, ISSUER_ID, APP_BUNDLE_ID, Environment.Production)
```
